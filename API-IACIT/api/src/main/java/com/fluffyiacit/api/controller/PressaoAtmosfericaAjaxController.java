...
import com.fluffyiacit.api.report.PdfPressaoAtmosferica;
...
//	        System.out.println("5:" + estDTfinal);
	...
	@GetMapping(value = "/PressaoAtmosferica/pdf/{estNome}/{estEstado}/{estDTinicial}/{estDTfinal}", produces = MediaType.APPLICATION_PDF_VALUE)
	public ResponseEntity<InputStreamResource> relatorioPressaoAtm (HttpServletResponse response,
																	 @PathVariable("estNome") String estNome,
																	 @PathVariable("estEstado") String estEstado,
																	 @PathVariable("estDTinicial") String estDTinicial,
		...
		List<ViewPressaoAtmModal> pressaoAtm = pressaoatmrepository.listRange(estEstado, estNome,Timestamp.valueOf(estDTinicial),Timestamp.valueOf(estDTfinal));

		for (ViewPressaoAtmModal objviewPressao : pressaoAtm) {
			if (objviewPressao.getPressaoAtmEstacao() == null) {
				objviewPressao.setPressaoAtmEstacao("N/A");
			}
			if (objviewPressao.getPressaoAtmMax() == null) {
		...
		ByteArrayInputStream bis = PdfPressaoAtmosferica.exportarPdfPressaoAtm(pressaoAtm);

		HttpHeaders headers = new HttpHeaders();

		headers.add("Content-Disposition", "attachment;filename=Relatório Pressão Atmosférica " +
				estNome + "(" + new SimpleDateFormat("dd-MM-yyyy")
				.format(pressaoAtm.get(0).getDatahoraCaptacao()) + " até " +
				new SimpleDateFormat("dd-MM-yyyy").format(pressaoAtm.get(pressaoAtm.size() - 1).getDatahoraCaptacao()) +
				").pdf");
