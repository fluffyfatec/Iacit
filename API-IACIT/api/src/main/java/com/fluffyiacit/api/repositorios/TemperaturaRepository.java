...
public interface TemperaturaRepository   extends JpaRepository<ViewTemperaturaModal, Integer>{
	...
	@Query ("SELECT t FROM view_temperatura_dados t WHERE t.estacao_estado = :estacao_estado AND t.estacao_nome = :estacao_nome AND t.cod_wmo = :cod_wmo AND t.datahora_captacao = :datahora_captacao")
