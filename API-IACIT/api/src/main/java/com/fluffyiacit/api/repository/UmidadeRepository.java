...
public interface UmidadeRepository   extends JpaRepository<ViewUmidadeModal, String> {
	...
	//LISTA 1 DIA
	@Query ("SELECT pa FROM view_umidade_dados pa WHERE pa.estacaoEstado = :estacaoEstado AND pa.estacaoNome = :estacaoNome AND pa.datahoraCaptacao = :datahoraCaptacao")
	public List<ViewUmidadeModal> listUnica(@Param("estacaoEstado") String estacaoEstado,
    ...
    public List<ViewUmidadeModal> listRange(@Param("estacaoEstado") String estacaoEstado, 
									    	@Param("estacaoNome") String estacaoNome,
									    	@Param("dataHoraInicial") Timestamp dataHoraInicial,
									    	@Param("dataHoraFinal") Timestamp dataHoraFinal);
	
//	@Query(value = "SELECT * FROM view_umidade_dados pa WHERE pa.estacao_estado = :estacaoEstado AND pa.estacao_nome = :estacaoNome AND pa.datahora_captacao BETWEEN (CAST(:datahoraCaptacao AS TIMESTAMP) - INTERVAL '7 days') AND CAST(:datahoraCaptacao AS TIMESTAMP)", nativeQuery = true)
