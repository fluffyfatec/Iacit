...
import com.fluffyiacit.api.report.PdfVento;
...
public class VentoAjaxController {
    ...
    @GetMapping(value = "/Ventos/pdf/{estNome}/{estEstado}/{estDTinicial}/{estDTfinal}", produces = MediaType.APPLICATION_PDF_VALUE)
    public ResponseEntity<InputStreamResource> relatorioVento (HttpServletResponse response,
                                                                     @PathVariable("estNome") String estNome,
                                                                     @PathVariable("estEstado") String estEstado,
                                                                     @PathVariable("estDTinicial") String estDTinicial,
        ...
        List<ViewVentoModal> vento = ventoRepository.listRange(estEstado, estNome,Timestamp.valueOf(estDTinicial),Timestamp.valueOf(estDTfinal));

        for (ViewVentoModal objviewVento : vento) {
            if (objviewVento.getVentoDirecaoHorario() == null) {
                objviewVento.setVentoDirecaoHorario("N/A");
            }
            if (objviewVento.getVentoRajadaMax() == null) {
        ...
        ByteArrayInputStream bis = PdfVento.exportarPdfVento(vento);

        HttpHeaders headers = new HttpHeaders();

        headers.add("Content-Disposition", "attachment;filename=Relatório Ventos " +
                estNome + "(" + new SimpleDateFormat("dd-MM-yyyy")
                .format(vento.get(0).getDatahoraCaptacao()) + " até " +
                new SimpleDateFormat("dd-MM-yyyy").format(vento.get(vento.size() - 1).getDatahoraCaptacao()) +
                ").pdf");
    ...
    @RequestMapping(value = { "/filtroVen" }, method = RequestMethod.POST)
    public ModelAndView PaginaIndexFiltrada(FiltroDatasDTO filtrodatas) {
        ...
        filtrodatas.setPagina("/filtroVen");
        modelAndView.addObject("filtro", filtrodatas);
